window.addEventListener("DOMContentLoaded", () => {
  fetch("product.json")
    ...
    .then((data) => {
      ...
      const wrapper = document.querySelector(".PRODUCT-INNER-WRAPPER-3");

      if (wrapper) {
        // If wrapper exists, we're on the main page -> Render products
        ...
        document.querySelectorAll(".Heart").forEach((heart) => {
          heart.addEventListener("click", (event) => {
            ...
            if (!savedProducts.some((item) => item.id === productId)) {
            ...
            window.location.href = "hellprac.html"; // Redirect to saved product page
          });
        });
      }

      // If we are on hellprac.html, render all saved products
        ...
        container.innerHTML = savedProducts
          .map(
            (product) => `
              <div class="PRODUCT">
                  <img src="${product.image}" alt="${product.title}" 
                       onerror="this.src='fallback.jpg'" style="width: 200px;">
                  <h2>${product.title}</h2>
